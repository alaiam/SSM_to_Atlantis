...
Nyear = 2011
if (Velma){
  filename <- paste0("VELMA/",Nyear,"/regular_grid_PCB_WC_velma_",Nyear,".nc")
  filename_sed <- paste0("VELMA/",Nyear,"/regular_grid_PCB_sed_velma_",Nyear,".nc")
  output_path <- paste0(path, "intermediate output archive/output_VELMA_",Nyear,"_PCB/")
  
}else{
  filename <- paste0("No_VELMA/",Nyear,"/regular_grid_PCB_WC_novelma_",Nyear,".nc")
  filename_sed <- paste0("No_VELMA/",Nyear,"/regular_grid_PCB_sed_novelma_",Nyear,".nc")
  output_path <- paste0(path, "intermediate output archive/output_No_VELMA_",Nyear,"_PCB/")
}
...
roms_sed_vars <- tidync::hyper_grids(roms) %>% # all available grids in the ROMS ncdf
  pluck("grid") %>% # for each grid, pull out all the variables associated with that grid and make a reference table
  purrr::map_df(function(x){
    roms %>% tidync::activate(x) %>% tidync::hyper_vars() %>% 
      dplyr::mutate(grd=x)
...
PCB_WC_dim <- roms_vars %>% dplyr::filter(name==c("PCB_WC")) %>% pluck('grd')
PCB_sed_dim <-roms_vars %>% dplyr::filter(name==c("PCB_sed")) %>% pluck('grd')

variable_before_Atlantis2 <- roms %>%
  tidync::activate(PCB_WC_dim) %>%
  tidync::hyper_tibble(force = TRUE) %>%
  dplyr::select(PCB_WC, PCB_POC, PCB_DOC, longitude, latitude, sigma_layer,time)%>%
  dplyr::rename(
    PCB_WC=PCB_WC, 
    PCB_POC=PCB_POC, 
    PCB_DOC=PCB_DOC, 
...
variable_before_Atlantis_sed2 <- roms %>%
  tidync::activate(PCB_WC_dim) %>%
  tidync::hyper_tibble(force = TRUE) %>%
  dplyr::select(PCB_sed, PCB_sed_POC, PCB_sed_DOC, longitude, latitude,time)%>%
  dplyr::rename(
    PCB_sed=PCB_sed, 
    PCB_sed_POC=PCB_sed_POC, 
    PCB_sed_DOC=PCB_sed_DOC, 
    longitude = longitude,  
    latitude = latitude,  
    time = time)
...
foreach(days = step_file) %dopar%{
  ...
  variable_sed <- merge(box_composition, variable_before_Atlantis_sed, by = c("latitude", "longitude"))
  ###################################################################
  ...
  for (i in 0:(box-1)){
    for (t in 1:length(time)){
      ...
      sed_value <- (mean(subset$PCB_sed, na.rm = T)*1000)[[1]]
      ...
      sed_value <- (mean(subset$PCB_sed_DOC, na.rm = T)*1000)[[1]]
      ...
      sed_value <- (mean(subset$PCB_sed_POC, na.rm = T)*1000)[[1]]
