#################################
# From SSM regular grid NetCDF outputs to Temperature and salinity nc file for Atlantis
#################################
...
setwd(path)
output_path <- paste0("/outputTS_",year,"/")
if (!file.exists(output_path)){dir.create(output_path)}
if (velma = T){
  filename <- paste0("regular_grid_variables_",year,"_TS_velma.nc")
}else{
  filename <- paste0("regular_grid_variables_",year,"_TS.nc")
}
...
box_composition <- read.csv(paste0(path, dir_code, "/box_composition.csv"))
